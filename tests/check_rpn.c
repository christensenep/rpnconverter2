...
START_TEST(i2p_error_on_invalid_character_within_expression)
{
    char* desiredPostfixPointer = NULL;
    char* actualPostfixPointer = rpn_infix_to_postfix("a+B");
    ck_assert_ptr_eq(actualPostfixPointer, desiredPostfixPointer);
}
END_TEST
...
START_TEST(p2i_should_handle_chain_of_operators)
{
    char* desiredInfixString = "a+(b+c)";
    char* actualInfixString = rpn_postfix_to_infix("abc++");
    ck_assert_str_eq(actualInfixString, desiredInfixString);
    free(actualInfixString);
}
END_TEST
...
START_TEST(p2i_error_on_excess_operands)
{
    char* desiredInfixPtr = NULL;
...
START_TEST(p2i_error_on_operator_at_beginning_of_expression)
{
    char* desiredInfixPtr = NULL;
    char* actualInfixPtr = rpn_postfix_to_infix("*ab-cd*");
    ck_assert_ptr_eq(actualInfixPtr, desiredInfixPtr);
...
Suite* rpn_test_suite(void)
{
    ...
    tcase_add_test(tcase_infix_to_postfix_errors, i2p_error_on_invalid_character_within_expression);
    ...
    tcase_add_test(tcase_postfix_to_infix_errors, p2i_error_on_operator_at_beginning_of_expression);
